...
function programsReducer(
  state: ProgramsStateInterface = programsInitialState,
  action: AnyAction,
): ProgramsStateInterface {
  switch (action.type) {
...
const reorganizeDataAsMap = (
  previousState: ReviewsStateInterface,
  { programId, reviews }: ReviewsActionPayload,
): ReviewsStateInterface['programsReviews'] => {
...
function reviewsReducer(state: ReviewsStateInterface = reviewsInitialState, action: AnyAction): ReviewsStateInterface {
  switch (action.type) {
    ...
    case GET_PROGRAM_REVIEWS_SUCCESS:
      return {
        ...state,
        programsReviews: reorganizeDataAsMap(state, action.payload),
        programsScore: calculateRatings(action.payload),
      };
    default:
      return state;
  }
