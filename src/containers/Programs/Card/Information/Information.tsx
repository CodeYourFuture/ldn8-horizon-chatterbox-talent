...
const Information = ({ description, locations, keyFacts, stepsToApply, careerType }: Partial<ProgramInterface>) => {
  ...
  return (
    <InformationWrapper>
      <TipsWrapper>
        <h2>Career types:</h2>
        {careerType &&
          careerType.map((career, index) => (
            <Tip key={index} color={styles['background-blue-analogous']}>
              {career}
            </Tip>
          ))}
      </TipsWrapper>
      <TipsWrapper>
        ...
        {locations &&
          locations.map((location, index) => (
            <Tip key={index} color={styles['background-beige-triadic']}>
              {location}
            </Tip>
          ))}
      </TipsWrapper>
      ...
      <RegularWrapper>
        ...
        <CleanList>
          {keyFacts?.map((keyFact, index) => (
            <li key={index}>{keyFact}</li>
          ))}
        </CleanList>
      </RegularWrapper>
      <RegularWrapper>
        <h2>Steps to apply</h2>
        <CleanList>{parseArrayWithIndexIntoList(stepsToApply)}</CleanList>
      </RegularWrapper>
    </InformationWrapper>
  );
};
